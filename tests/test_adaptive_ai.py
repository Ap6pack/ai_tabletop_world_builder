...
def create_test_game_state(status="completed", include_business_impact=True):
    ...
    timeline = [
        IncidentEvent(
            ...
            description="SIEM alert: suspicious login",
            severity="high",
            actor="system",
        ),
        IncidentEvent(
            ...
            event_type="action",
            description="Check SIEM logs for alert details",
            severity="medium",
            actor="player",
        ),
        IncidentEvent(
            ...
            description="Alert confirmed as brute force",
            severity="info",
            actor="system",
        ),
        IncidentEvent(
            ...
            event_type="action",
            description="Investigate source IP and correlate",
            severity="medium",
            actor="player",
        ),
        IncidentEvent(
            ...
            event_type="action",
            description="Block the malicious IP at firewall",
            severity="medium",
            actor="player",
        ),
        IncidentEvent(
        ...
        IncidentEvent(
            ...
            description="Threat actor pivoted to phishing",
            severity="high",
            actor="threat_actor",
        ),
        IncidentEvent(
            ...
            event_type="action",
            description="Monitor email gateway for phishing",
            severity="medium",
            actor="player",
        ),
        IncidentEvent(
        ...
        IncidentEvent(
            ...
            event_type="consequence",
            description="Workstation isolated, movement stopped",
            severity="low",
            actor="system",
        ),
        IncidentEvent(
    ...
    objectives = [
        Objective(
            ...
            type="investigate",
            success_criteria="Determine initial access",
            points=25,
            difficulty="easy",
            status="completed",
        ),
...
def test_adaptive_difficulty():
    ...
    print(
        f"  PASS - Skill profile: overall={skill['overall_skill']}, "
...
def test_training_path():
    ...
    for gap in gaps:
        for key in ("skill_area", "current_level", "target_level", "gap", "priority", "recommended_scenarios"):
        ...
        assert gap["gap"] >= 0
    print(f"  PASS - Found {len(gaps)} skill areas, {sum(1 for g in gaps if g['priority'] == 'high')} high-priority")
    ...
    print(f"  PASS - Recommended: {rec['scenario_type']} ({rec['difficulty']}), focus={rec['focus_areas']}")
    ...
    print(
        f"  PASS - Plan: current={plan['current_level']}, target={plan['target_level']}, "
    ...
    print(
        f"  PASS - Progress: {progress['sessions_completed']} sessions, "
    ...
    print(
        f"  PASS - Single session: gaps={len(single_gaps)}, "
