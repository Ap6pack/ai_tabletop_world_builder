...
DIFFICULTY_CONFIGS = {
    "easy": {"action_point_bonus": 2, "timer_multiplier": 1.3,
             "threat_aggression_cap": 50, "hints_enabled": True,
             "resource_multiplier": 1.2},
    "medium": {"action_point_bonus": 0, "timer_multiplier": 1.0,
               "threat_aggression_cap": 80, "hints_enabled": True,
               "resource_multiplier": 1.0},
    "hard": {"action_point_bonus": -2, "timer_multiplier": 0.7,
             "threat_aggression_cap": 100, "hints_enabled": False,
...
class AdaptiveDifficultyService:
    ...
    def calculate_player_skill(
        ...
        evaluations: Optional[List[DecisionEvaluation]] = None,
    ) -> Dict:
        ...
        obj_ratio = (sum(1 for o in game_state.objectives if o.status == "completed")
                     / total_obj * 100) if total_obj else 50.0
        if evaluations:
        ...
        return {"overall_skill": overall, "category_skills": category_skills,
                "trend": trend, "recommended_difficulty": rec}
    ...
    def should_adjust(self, game_state: GameState) -> bool:
        ...
        actions = sum(1 for e in game_state.incident_timeline
                      if e.actor == "player" and e.event_type == "action")
        if actions < MIN_ACTIONS_FOR_ADJUSTMENT:
            return False
        if self._last_adjustment_time is not None:
            elapsed = (datetime.now(timezone.utc) - self._last_adjustment_time).total_seconds() / 60
    ...
    def _calculate_recent_performance(self, game_state: GameState) -> float:
        ...
        indices = [i for i, e in enumerate(timeline)
                   if e.actor == "player" and e.event_type == "action"]
        if not indices:
            return 50.0
    ...
    def _detect_trend(self, game_state: GameState) -> str:
        ...
        indices = [i for i, e in enumerate(game_state.incident_timeline)
                   if e.actor == "player" and e.event_type == "action"]
        if len(indices) < 4:
            return "stable"
        mid = len(indices) // 2
